...
export function RewardsPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useRewards({ page }, { search, status: statusFilter });
  ...
  const lowStockCount = rewards.filter(
    (r) =>
      r.inventory_remaining != null &&
      r.inventory_total != null &&
      r.inventory_remaining <= r.inventory_total * 0.2,
  ).length;
  ...
  return (
    <div>
      <PageHeader
        title="Rewards"
        subtitle="Manage FET redemption offers"
        actions={
          <UnavailableActionButton
            icon={<Plus size={16} />}
            label="Create Reward"
            title="Reward creation is not live in this build."
          />
        }
      />
      ...
      <div className="grid grid-4 gap-4 mb-6">
        <KpiCard
          label="Total Rewards"
          value={result?.count ?? rewards.length}
          icon={<Gift size={18} />}
        />
        <KpiCard
          label="Active"
              ...
              {rewards.map((r) => (
                ...
                >
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted mono">{r.id}</div>
                      </div>
                    </div>
                  </td>
                  <td>
                  ...
                  <td className="mono font-semibold">
                    {formatFET(r.fet_cost)}
                  </td>
                  <td>
                    {r.inventory_total
                      ? `${r.inventory_remaining}/${r.inventory_total}`
                      : "∞"}
                  </td>
        ...
        {selected && (
          <>
            <DrawerSection title="Reward Info">
              ...
              <DrawerField
                label="FET Cost"
                value={formatFET(selected.fet_cost)}
              />
              <DrawerField
                label="Original Value"
                value={selected.original_value || "—"}
              />
              ...
              <DrawerField
                ...
                value={
                  <StatusBadge
                    status={selected.is_active ? "active" : "archived"}
                  />
                }
              />
              <DrawerField
                label="Featured"
                value={selected.is_featured ? "⭐ Yes" : "No"}
              />
            </DrawerSection>
            <DrawerSection title="Inventory">
              <DrawerField
                label="Total"
                value={selected.inventory_total ?? "∞"}
              />
              <DrawerField
                label="Remaining"
                value={selected.inventory_remaining ?? "∞"}
              />
              {selected.inventory_total &&
                selected.inventory_remaining != null && (
                  <div
                    className="mt-2"
                    style={{
                  ...
                  >
                    <div
                      style={{
                        width: `${(selected.inventory_remaining / selected.inventory_total) * 100}%`,
                        height: "100%",
                        background:
                          selected.inventory_remaining <=
                          selected.inventory_total * 0.2
                            ? "var(--fz-error)"
                            : "var(--fz-primary)",
                        borderRadius: 4,
            ...
            <DrawerSection title="Validity">
              <DrawerField
                label="Valid From"
                value={
                  selected.valid_from ? formatDate(selected.valid_from) : "—"
                }
              />
              <DrawerField
                label="Valid Until"
                value={
                  selected.valid_until ? formatDate(selected.valid_until) : "—"
                }
              />
            </DrawerSection>
