// FANZONE Admin — Settle Pool Modal
import { useState } from 'react';
import { Target } from 'lucide-react';

interface SettlePoolModalProps {
  open: boolean;
  poolId: string;
  matchLabel: string;
...
export function SettlePoolModal({ open, poolId, matchLabel, onConfirm, onCancel, isPending }: SettlePoolModalProps) {
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-panel" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-md font-semibold">Settle Pool</h3>
            <p className="text-xs text-muted mt-1">{poolId} — {matchLabel}</p>
          </div>
        </div>

        <p className="text-sm text-muted mb-4">
          Enter the official final score to settle this pool. Winners will be determined by exact score match
          and FET will be distributed accordingly. This action cannot be undone.
        </p>
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {isPending ? 'Settling...' : 'Settle Pool'}
          </button>
        </div>
      </div>
    </div>
