...
FANZONE is a production sports-bar entertainment platform. Guests discover venues in the mobile app, browse a venue menu, place orders, receive non-cash FET loyalty rewards, join Arena challenges, and use FET loyalty points where a venue allows it.
...
- FET non-cash loyalty ledger;
- match curation;
- challenge creation, joining, live stats, sharing, and settlement;
...
Flutter mobile app
  ...
  -> Supabase client
  -> guest ordering, loyalty, challenges, profile, notifications

Website PWA
  -> Vite, React, Zustand
  -> Supabase browser client and Edge Functions
  -> public guest web, in-app ordering, challenges, loyalty

Venue portal
  -> Vite, React
  -> Supabase browser client, RPCs, Edge Functions
  -> orders, menu, pools, FET rewards, insights

Admin console
  -> Vite, React
  -> Supabase browser client, admin RPCs, Edge Functions
  -> curation, platform controls, loyalty-ledger oversight, audit
...
| FET loyalty ledger | Legacy `fet_wallets` and `fet_wallet_transactions` tables; no duplicate loyalty-balance state. |
...
- Settlement is idempotent and loyalty-ledger backed.
