...
| `dart format --output=none --set-exit-if-changed lib test integration_test tool` | Passed | Formatting drift fixed; rerun completed with 0 changes. |
| `flutter analyze` | Passed | Analyzer drift fixed; rerun passed with no issues. |
...
| `deno test --allow-env supabase/functions` | Passed | 32 tests passed. |
...
| `npm audit --audit-level=moderate` in `scripts` | Passed | Advisory cleared after `npm audit fix` removed stale lockfile dependencies. |
| `bash -n tool/deploy_cloudflare_pages.sh tool/*.sh` | Passed | Shell syntax valid after deploy guard update. |
| `git diff --check` | Passed | Final implementation diff has no whitespace errors. |
...
- Branch protection configuration for the now-enabled CI checks.
- Production Supabase migration application and rollback plan.
- MFA or server-mediated controls for the most sensitive admin and venue actions.
- Deployed Edge Function CORS allowlist and environment-specific origins.
...
4. Rerun Android release build and iOS no-codesign build in the supported release environment.
5. Run browser smoke tests against deployed web apps and verify CSP headers, Edge Function CORS, service worker behavior, and admin/venue auth flows.
