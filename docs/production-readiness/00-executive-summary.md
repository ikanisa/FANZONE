...
| P2 | Public URL rendering | Browser apps now use shared `safeHref`/`safeImageUrl` helpers at public link and image boundaries. | `packages/core/src/url.ts`, `apps/website/src/components/MatchPools.tsx`, `apps/venue-portal/src/features/settings/QRFactoryPage.tsx` | DB-level URL constraints are still not enforced for all externally sourced URL columns. | Add database constraints for generated URL fields where feasible. | Fixed in app code |
