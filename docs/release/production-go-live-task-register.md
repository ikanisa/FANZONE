...
| P1-06 | Complete critical user-flow UAT. | QA owner | Evidence for anonymous session, WhatsApp OTP upgrade, wallet, ordering, MoMo/Revolut external handoff, "I paid", staff confirmation, pools, rewards, admin moderation, TV live display, realtime propagation, and backend isolation. | `release/qa/critical-user-flow-uat.json`; `node tool/validate_critical_uat_signoff.mjs`; `docs/release/go-live-checklist.md` |
| P1-07 | Verify production observability. | Operations owner | Sentry or equivalent DSNs configured, alert routes, dashboard links, and post-deploy watch schedule. | `tool/collect_world_class_evidence.sh`; `SENTRY_DSN`; provider dashboard evidence |
