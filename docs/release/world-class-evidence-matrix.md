...
| Production observability and alerting | PENDING | PENDING | PENDING | PENDING | `release/operations/operations-readiness-evidence.json`; `node tool/validate_operations_readiness_evidence.mjs`; `docs/operations/scheduler-observability.md`; `tool/collect_world_class_evidence.sh`; Sentry/equivalent, dashboards, alert routes, and post-deploy watch evidence required |
| Incident response and rollback readiness | PENDING | PENDING | PENDING | PENDING | `release/operations/operations-readiness-evidence.json`; `node tool/validate_operations_readiness_evidence.mjs`; incident owner, escalation channel, rollback tag, DB restore plan, runbook review, post-deploy watch, and sample alert test required |
...
| Load and reliability smoke | PENDING | PENDING | PENDING | PENDING | `release/performance/load-reliability-evidence.json`; `node tool/validate_load_reliability_evidence.mjs`; ordering, staff calls, FET ledger, rewards, entertainment, admin queues, TV recovery, realtime, Edge Function error-budget, and RLS-under-load evidence required |
