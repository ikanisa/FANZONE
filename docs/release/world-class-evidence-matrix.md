...
| Android release artifact | PARTIAL | N/A | N/A | N/A | `release/android/android-release-readiness.json`; `node tool/validate_android_release_evidence.mjs`; `docs/release/go-live-current-state-2026-06-02.md`; fresh signed AAB/APK, APK signature verification, artifact freshness, and physical-device install/launch smoke are captured locally; Play app-signing/internal-test evidence, deep-link smoke, full core-flow smoke, review metadata confirmation, and owner approval remain required |
...
| Privacy/legal review | PENDING | PENDING | PENDING | PENDING | `release/legal/privacy-legal-readiness-evidence.json`; `node tool/validate_privacy_legal_readiness_evidence.mjs`; source-commit-bound production URLs, review window, durable privacy/legal evidence bundle, exact check-to-surface mapping, public policy URL, Google Play Data Safety, Apple privacy labels, retention, deletion, export, support access, SDK inventory, no-betting/no-cash-out/off-platform-payment wording, and human legal review evidence required |
