      ...
      {activeTab === 'Fan Clubs' ? (
        ...
        <>
          ...
          <div className="flex justify-center items-end gap-2 p-6 bg-surface2 border-b border-border">
            <PodiumItem rank={2} name="290145" fet="12.4k" height="h-28" />
            <PodiumItem rank={1} name="883201" fet="15.2k" height="h-36" />
            <PodiumItem rank={3} name="451992" fet="10.1k" height="h-24" />
          </div>

          {/* List */}
          <div className="p-3 space-y-2">
            {[4, 5, 6, 7, 8].map((rank) => (
              <LeaderboardRow key={rank} rank={rank} name={`${100000 + rank * 15321}`.substring(0, 6)} fet={`${10 - rank}.5k`} />
            ))}
          </div>
