...
export function MatchCard({
  ...
  return (
    <Card
      className={`p-3 hover:border-accent/30 group transition-all relative overflow-hidden ${live ? "border-danger/30 shadow-[0_0_20px_rgba(239,68,68,0.05)]" : ""}`}
    >
      ...
      <Link to={`/match/${matchId}`} className="block relative z-10">
        <div className="flex justify-between items-center mb-3">
          <div className="flex flex-col items-center gap-1.5 w-[35%]">
            ...
            <span className="font-bold text-[10px] text-center leading-tight truncate px-1 w-full">
              {home}
            </span>
          </div>
          <div className="w-[30%] flex justify-center">
            <div
              className={`font-mono text-xl font-bold tracking-tight ${live ? "text-danger [text-shadow:0_0_15px_rgba(239,68,68,0.2)]" : "text-text"}`}
            >
              {live ? score : "VS"}
            </div>
          </div>
