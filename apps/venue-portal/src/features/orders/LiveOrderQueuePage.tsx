...
const paymentMethods: SupportedPaymentMethod[] = [
  "momo",
  "revolut",
  "cash",
  "other",
...
function initialDraft(order: Order): PaymentDraft {
  ...
  return {
    status,
    method: order.paymentMethod === "card" ? "other" : order.paymentMethod,
    note: "",
  };
}
  ...
  return (
    <article className="ops-card overflow-hidden">
      <div className="p-5 border-b border-border flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="lg:text-right">
          ...
          <p className="text-xs font-bold text-textSecondary mt-1">
            +{order.fetEarned.toLocaleString()} FET earned |{" "}
            {order.fetSpent.toLocaleString()} FET spent
          </p>
        </div>
      </div>
      ...
      <div className="p-5 grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6">
        <div>
          <p className="text-[10px] font-black text-textSecondary uppercase tracking-widest mb-3">
            Items
          </p>
          <div className="space-y-2">
            {order.items?.length ? (
              order.items.map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between gap-4 rounded-xl bg-surface2 px-4 py-3"
                >
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-2">
              <span className="text-[10px] font-black text-textSecondary uppercase tracking-widest">
                Payment
              </span>
              <select
                className="input"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button
              ...
              disabled={busy}
              onClick={() => onPaymentStatus(draft.status)}
            >
              Save payment status
            </button>
          </div>
    ...
    <section className="ops-card border-warning/30 bg-warning/10 p-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-text">
              {bells.length} active bell{bells.length === 1 ? "" : "s"}
            </h2>
          </div>
        </div>
        <p className="text-sm font-bold text-textSecondary">
          Acknowledge requests once staff has responded.
        </p>
      </div>
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-2">
        {bells.map((bell) => (
          <article
            key={bell.id}
            className="rounded-2xl border border-border bg-surface/80 p-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"
          >
            <div>
              <p className="text-lg font-black text-text">
                Table{" "}
                {bell.tableNumber ?? bell.tableId.slice(0, 6).toUpperCase()}
              </p>
              <p className="text-sm font-bold text-textSecondary">
                {ageLabel(bell.createdAt)} ago · User{" "}
                {bell.userId.replace(/-/g, "").slice(-6).toUpperCase()}
              </p>
              {bell.message && (
...
export const LiveOrderQueuePage: React.FC = () => {
    ...
    try {
      ...
      setActionError(
        err instanceof Error
          ? err.message
          : "Could not acknowledge bell request.",
      );
    } finally {
      setBusyBellId(null);
    }
  ...
  async function confirmPaid(details: {
    ...
    try {
      ...
      setMarkPaidError(
        err instanceof Error ? err.message : "Could not mark order paid.",
      );
    } finally {
      setBusyId(null);
    }
    ...
    <div className="space-y-6 max-w-[1500px] mx-auto">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        >
          <RefreshCcw
            size={16}
            className={loading || bellsLoading ? "animate-spin" : ""}
          />
          Refresh
        </button>
      </div>
