...
import { VenueStakes } from './VenueStakes';
...
export default function MatchDetail() {
  ...
  const [stakes, setStakes] = useState<Array<VenueMatchStake & { venueName: string }>>([]);
  ...
  useEffect(() => {
    ...
    async function loadMatch() {
      ...
      const [nextEngine, nextFormRows, nextConsensus, nextPrediction, allStandings, nextStakes] =
        await Promise.all([
          ...
          api.fetchActiveStakesForMatch(id),
        ]);
        ...
        <div className="px-6 mb-6 space-y-4">
          {stakes.map((stake) => (
            <VenueStakes
              ...
              venueId={stake.venueId}
              venueName={stake.venueName}
            />
          ))}
        </div>
      )}
