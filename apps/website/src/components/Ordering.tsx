...
import type { Venue, MenuItem, MenuCategory, Order } from "../types";
...
export const Ordering: React.FC = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 text-center">
        ...
        <button
          ...
          className="px-8 py-3 bg-primary text-primaryText rounded-xl font-bold"
        >
          Retry
        </button>
      </div>
  ...
  if (orderStatus === "success" && placedOrder) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
        ...
        <p className="text-textSecondary mb-8">
          Order #{placedOrder.orderCode} is in the venue queue.
        </p>
          ...
          {placedOrder.paymentFetAmount > 0 && (
            <div className="flex justify-between text-success">
              ...
              <span className="font-bold">
                {placedOrder.paymentFetAmount} FET
              </span>
            </div>
          )}
        </Card>
  ...
  const venueCoverUrl = safeImageUrl(venue.coverUrl);
    ...
    <div className="relative min-h-screen pb-32">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-6 right-6">
          <h1 className="text-3xl font-black text-text drop-shadow-lg">
            {venue.name}
          </h1>
          <div className="flex items-center gap-2 mt-1">
            <Badge variant={venue.isOpen ? "success" : "secondary"}>
                ...
                {cart.find((i) => i.id === item.id) ? (
                  <div className="flex items-center bg-surface2 rounded-xl border border-border overflow-hidden">
                    <button
                      ...
                      className="p-2 hover:bg-surface3 text-text"
                    >
                      <Minus size={16} />
                    </button>
                    <span className="w-8 text-center font-bold text-text">
                      {cart.find((i) => i.id === item.id)?.quantity}
                    </span>
                    <button
                      onClick={() => addToCart(item)}
                      className="p-2 hover:bg-surface3 text-primary"
                    >
                      <Plus size={16} />
                    </button>
                  </div>
        ...
        {totalItemCount > 0 && (
          ...
          >
            ...
            >
              ...
              <span className="flex-1 text-center font-black text-sm tracking-wider uppercase">
                VIEW CART
              </span>
              <span className="font-black text-lg">
                {venue?.country === "RW" ? "" : "€"}
                {total.toLocaleString()} {venue?.country === "RW" ? "RWF" : ""}
              </span>
            </button>
          </motion.div>
        )}
            ...
            >
              <div className="p-8">
                <div className="flex justify-between items-center mb-8">
                  ...
                  <button
                    onClick={() => setIsCheckoutSheetOpen(false)}
                    className="p-2 bg-surface2 rounded-full text-textSecondary hover:text-text"
                  >
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="bg-surface2 rounded-2xl border border-border p-4 mb-8">
                  <p className="text-xs font-bold text-accent2 uppercase tracking-wider">
                    FET rewards
                  </p>
                  <p className="text-sm font-medium text-textSecondary mt-1">
                    Earn FET after venue staff manually confirms payment. Tokens
                    are not redeemed as checkout payment.
                  </p>
                </div>
