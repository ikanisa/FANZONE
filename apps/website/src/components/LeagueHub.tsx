...
export default function LeagueHub() {
  ...
  const predictionRoute = getPlatformFeatureRoute('predictions', {
    fallback: '/fixtures',
  });
  ...
  return (
    <div className="min-h-screen bg-bg pb-24 transition-colors duration-300">
      ...
      >
        <Link to={fixturesRoute} className="text-text hover:text-accent transition-all">
          <ChevronLeft size={24} />
        </Link>
      ...
      <div className="bg-surface2 p-6 border-b border-border relative overflow-hidden">
        ...
        {(showPredictions || showLeaderboard) && (
          <div className="flex gap-3 mt-6 relative z-10">
            {showPredictions && (
              <button
                onClick={() => navigate(predictionRoute)}
            ...
            {showLeaderboard && (
              <button
                onClick={() => navigate(leaderboardRoute)}
      ...
      <div className="p-4 space-y-8">
        <section>
          <div className="flex items-center justify-between mb-3 px-2">
            ...
            <Link to={fixturesRoute} className="text-xs font-bold text-accent">
              SEE ALL
            </Link>
          </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3 px-2">
            ...
            {showPredictions && (
              <Link to={predictionRoute} className="text-xs font-bold text-accent">
                OPEN PREDICT
              </Link>
            )}
          </div>
