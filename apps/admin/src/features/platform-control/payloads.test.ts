  ...
  it("normalizes feature payloads for the admin RPC", () => {
    const payload = buildPlatformFeaturePayload({
      ...
      web_channel: {
        ...
        entry_key: " predictions.web ",
        navigation_label: " Predictions ",
        placement_key: " home-primary ",
        metadata: { variant: "hero" },
      },
    });

    expect(payload.feature_key).toBe("predictions");
    expect(payload.default_route_key).toBe("/predict");
    expect(payload.mobile_channel.route_key).toBe("/predict");
    expect(payload.mobile_channel.entry_key).toBe("predictions.index");
    expect(payload.web_channel.route_key).toBe("/match/:matchId");
