...
function fromContentBlockRecord(
  ...
  return {
    ...
    metadata_text: toJsonText(block.metadata),
  };
}
...
export function PlatformControlPage() {
  ...
  const selectFeature = (feature: PlatformFeatureRecord | null) => {
    setSelectedFeatureKey(feature?.feature_key ?? null);
    setFeatureForm(feature ? fromFeatureRecord(feature) : emptyFeatureForm());
    setFeatureFormError(null);
  };

  const selectBlock = (block: PlatformContentBlockRecord | null) => {
    setSelectedBlockKey(block?.block_key ?? null);
    setBlockForm(block ? fromContentBlockRecord(block) : emptyContentBlockForm());
  ...
  const handleFeatureSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await upsertFeature.mutateAsync({
        ...
        role_restrictions: parseJsonValue(
          featureForm.role_restrictions_text,
          "Role restrictions",
          { allowArray: true },
        ),
        ...
        metadata: parseJsonObject(featureForm.metadata_text, "Feature metadata"),
  ...
  const handleBlockSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await upsertContentBlock.mutateAsync({
        ...
        metadata: parseJsonObject(blockForm.metadata_text, "Block metadata"),
      });
        ...
        <form className="space-y-4" onSubmit={handleFeatureSubmit}>
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label className="form-field md:col-span-2">
                <span>Role restrictions JSON</span>
                <textarea
                  className="textarea font-mono"
                  rows={4}
                  value={featureForm.role_restrictions_text}
                  onChange={(event) =>
                    setFeatureForm((current) => ({
                      ...current,
                      role_restrictions_text: event.target.value,
                    }))
                  }
                />
              </label>
              ...
              <label className="form-field md:col-span-2">
                <span>Feature metadata JSON</span>
                <textarea
                  className="textarea font-mono"
                  rows={4}
                  value={featureForm.metadata_text}
                  onChange={(event) =>
                    setFeatureForm((current) => ({
                      ...current,
