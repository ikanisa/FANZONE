...
-- =====================================================================
-- 1. FIX ADMIN POLICY DRIFT FROM EARLIER LOCAL SCALABILITY MIGRATIONS
-- =====================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'team_supporters_user_id_auth_fkey'
  ) THEN
    ALTER TABLE public.team_supporters
      ADD CONSTRAINT team_supporters_user_id_auth_fkey
      FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE NOT VALID;
  END IF;
END $$;
ALTER TABLE public.team_supporters
  VALIDATE CONSTRAINT team_supporters_user_id_auth_fkey;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'team_contributions_user_id_auth_fkey'
  ) THEN
    ALTER TABLE public.team_contributions
      ADD CONSTRAINT team_contributions_user_id_auth_fkey
      FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE NOT VALID;
  END IF;
END $$;
ALTER TABLE public.team_contributions
  VALIDATE CONSTRAINT team_contributions_user_id_auth_fkey;
...
REVOKE ALL ON FUNCTION public.merge_anonymous_to_authenticated(uuid, uuid) FROM authenticated;
REVOKE ALL ON FUNCTION public.merge_anonymous_to_authenticated(uuid, uuid) FROM anon;
...
-- =====================================================================
-- 4. NORMALIZE TEAM ↔ COMPETITION RELATIONSHIP WITHOUT BREAKING APP MODELS
-- =====================================================================
...
SELECT
  ...
  t.is_active,
  t.is_featured,
  t.fet_contributions_enabled,
  t.fiat_contributions_enabled,
  t.fiat_contribution_mode,
  t.fiat_contribution_link,
  t.fan_count
FROM public.teams t
LEFT JOIN (
  SELECT
...
-- =====================================================================
-- 5. NORMALIZE GLOBAL CHALLENGE ↔ MATCH RELATIONSHIP WITH COMPAT VIEWS
-- =====================================================================
...
SELECT
  flag.key,
  lower(coalesce(nullif(flag.market, ''), 'global')),
  'all',
  coalesce(flag.is_enabled, false),
  coalesce(flag.description, flag.label),
  timezone('utc', now())
...
-- =====================================================================
-- 7. MATCH ALERT DELIVERY BACKEND
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS public.match_alert_dispatch_log (
  ...
  match_id text NOT NULL REFERENCES public.matches(id) ON DELETE CASCADE,
  alert_type text NOT NULL CHECK (alert_type IN ('kickoff', 'goal', 'result')),
  dispatch_key text NOT NULL,
  live_event_id uuid REFERENCES public.live_match_events(id) ON DELETE SET NULL,
