-- Additive manual/off-platform payment audit hardening.
-- Staff/admin confirmations for paid/refund/dispute-style states must include
-- an actor note so daily close and incident review have usable evidence.
...
DECLARE
  ...
  v_method text;
  v_actor_note text := nullif(trim(coalesce(p_actor_note, '')), '');
...
BEGIN
  ...
  IF v_next_status IN ('paid', 'partially_paid', 'refunded', 'disputed')
     AND v_actor_note IS NULL THEN
    RAISE EXCEPTION 'Actor note is required for manual payment status %', v_next_status;
  END IF;
...
IS 'Venue-scoped manual payment update with required actor notes for paid/refund/dispute-style states, amount/reference capture, payment_events logging, and no provider API execution.';
