...
BEGIN
  ...
  VALUES
    ('settlement_final_match', 'settlement_engine_comp', timezone('utc', now()) + interval '1 day', 'scheduled', 'settle_home', 'settle_away'),
    ('settlement_refund_match', 'settlement_engine_comp', timezone('utc', now()) + interval '1 day', 'scheduled', 'settle_home', 'settle_away'),
    ('settlement_cancelled_match', 'settlement_engine_comp', timezone('utc', now()) + interval '1 day', 'scheduled', 'settle_home', 'settle_away'),
    ('settlement_postponed_match', 'settlement_engine_comp', timezone('utc', now()) + interval '1 day', 'scheduled', 'settle_home', 'settle_away'),
    ('settlement_retry_match', 'settlement_engine_comp', timezone('utc', now()) + interval '1 day', 'scheduled', 'settle_home', 'settle_away');
  ...
  INSERT INTO public.match_pools (id, match_id, title, creator_user_id, entry_fee_fet, stake_min_fet, stake_max_fet)
  VALUES (v_pool_final, 'settlement_final_match', 'Final settlement pool', v_creator, 0, 1, 100);
  ...
  INSERT INTO public.match_pools (id, match_id, title, creator_user_id, entry_fee_fet, stake_min_fet, stake_max_fet)
  VALUES (v_pool_refund, 'settlement_refund_match', 'No winner refund pool', v_creator, 0, 1, 100);
  ...
  INSERT INTO public.match_pools (id, match_id, title, creator_user_id, entry_fee_fet, stake_min_fet, stake_max_fet)
  VALUES (v_pool_cancelled, 'settlement_cancelled_match', 'Cancelled refund pool', v_creator, 0, 1, 100);
  ...
  -- Postponed match: default rule holds stakes and exposes a pending run.
  INSERT INTO public.match_pools (id, match_id, title, creator_user_id, entry_fee_fet, stake_min_fet, stake_max_fet)
  VALUES (v_pool_postponed, 'settlement_postponed_match', 'Postponed hold pool', v_creator, 0, 1, 100);
  ...
  IF v_result ->> 'status' <> 'held' THEN
    RAISE EXCEPTION 'Expected postponed pool to be held, got %', v_result;
  END IF;
  ...
  IF v_tx_count <> 1 THEN
    RAISE EXCEPTION 'Expected one pending postponed settlement run, got %', v_tx_count;
  END IF;

  -- Failed run is recorded, then retry reuses the settlement row idempotently.
  INSERT INTO public.match_pools (id, match_id, title, creator_user_id, entry_fee_fet, stake_min_fet, stake_max_fet)
  VALUES (v_pool_retry, 'settlement_retry_match', 'Retry settlement pool', v_creator, 0, 1, 100);
  ...
  IF v_result ->> 'status' <> 'failed' THEN
    RAISE EXCEPTION 'Expected failed settlement for missing result camp, got %', v_result;
  END IF;
