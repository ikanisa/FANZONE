...
Deno.serve(async (req) => {
  ...
  const logger = createLogger({
    requestId,
    action: "admin_approve_onboarding",
  });
  ...
  try {
    ...
    if (requestError || !request) {
      logger.warn("Onboarding request not found", {
        requestId: input.request_id,
      });
    ...
    if (input.action === "approve") {
      ...
      if (request.revolut_link) {
        vendorUpdates.revolut_link = request.revolut_link;
      }
      ...
      if (userUpdateError) {
        logger.error("Failed to activate venue user", {
      ...
      if (request.menu_items_json && Array.isArray(request.menu_items_json)) {
        ...
        if (categoryError) {
          logger.warn("Failed to add menu categories", {
        ...
        const menuItems = request.menu_items_json
          .map((item: Record<string, unknown>, index: number) => {
            const categoryName = String(item.category || "Menu").trim() ||
              "Menu";
        ...
        if (menuItems.length > 0) {
          ...
          if (menuError) {
            logger.warn("Failed to add menu items", {
