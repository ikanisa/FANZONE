...
export async function callGemini(
    ...
    while (attempt < MAX_RETRIES) {
      try {
        ...
        if (
          ...
          if (attempt < MAX_RETRIES - 1) {
            ...
            console.warn(
              `Attempt ${
                attempt + 1
              } failed for ${modelName} (${response.status}), retrying in ${delay}ms...`,
            );
        ...
        if (attempt < MAX_RETRIES - 1) {
          const delay = RETRY_DELAY * Math.pow(2, attempt);
          console.warn(
            `Network error on ${modelName}, retrying in ${delay}ms...`,
            err,
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
          attempt++;
  ...
  try {
    ...
    if (fallbackModel) {
      console.warn(
        `Primary model ${primaryModel} failed after retries, trying fallback ${fallbackModel}`,
        error,
      );
      try {
        ...
        throw new Error(
          `Gemini API failed on primary and fallback: ${fallbackError}`,
        );
      }
    } else {
...
export async function searchWithGoogle(
  ...
  { results: Array<{ title: string; url: string; snippet: string }>; raw?: any }
> {
  const geoContext = options.geo === "RW"
    ? "Rwanda, East Africa"
    : options.geo === "MT"
    ? "Malta, Europe"
    : "";
  ...
  const prompt = `${query}${
    geoContext ? ` (focus on ${geoContext})` : ""
  ...
  if (groundingMeta?.groundingChunks) {
    for (
      const chunk of groundingMeta.groundingChunks.slice(
        0,
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (match?.[1]) {
          ...
          if (
            name.length > 2 && name.length < 100 && !name.startsWith("http")
          ) {
            places.push({ name });
...
export const AGENT_SYSTEM_PROMPTS: Record<AgentType, string> = {
  guest:
    `You are a friendly, knowledgeable AI waiter at a sports bar. Your role is to:
...
- Never make up menu items or prices`,
  ...
  bar_manager:
    `You are a professional business assistant helping bar/restaurant managers with daily operations. Your role is to:
...
- Prioritize urgent information first`,
  ...
  admin:
    `You are a platform management assistant helping FANZONE administrators. Your role is to:
...
export function buildAgentContext(context: {
  ...
  if (context.active_orders?.length) {
    const ordersStr = context.active_orders
      .map((o) =>
        `- Order ${o.id.slice(0, 8)}: ${o.status} - ${o.items.join(", ")}`
      )
      .join("\n");
...
export async function streamAgentChat(
  ...
  const url =
    `${GEMINI_API_URL}/models/gemini-2.0-flash:streamGenerateContent?key=${apiKey}`;
  const response = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
