  ...
  try {
    ...
    switch (input.action) {
      ...
      case "toggle_admin_status":
        return await handleToggleAdminStatus(
          supabaseAdmin,
          input,
          audit,
          logger,
...
async function handleCreateVendor(
  ...
  logger.info("Creating vendor with auth", {
    name: input.name,
  ...
  if (authError || !authData.user) {
    logger.error("Failed to create auth user for vendor", {
  ...
  // Generate a synthetic google_place_id for admin-created venues
  const googlePlaceId = `admin_created_${Date.now()}_${
  ...
  if (membershipError) {
    ...
    await supabaseAdmin.auth.admin.deleteUser(authData.user.id);
    logger.error("Failed to create vendor membership", {
...
async function handleUpdateVendor(
  ...
  if (input.revolut_link !== undefined) {
    updateData.revolut_link = input.revolut_link;
  }
  ...
  if (input.photos_json !== undefined) {
    updateData.photos_json = input.photos_json;
  }
...
async function handleToggleAdminStatus(
  ...
  logger.info("Toggling admin status", {
