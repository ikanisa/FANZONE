...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
      ...
      if (wallet.availableFet < fetSpendAmount) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Insufficient FET. You have ${wallet.availableFet} FET available.',
              ),
            ),
          );
        }
    ...
    if (order != null && mounted) {
        ...
        try {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Order placed, but FET spend was not applied: $error',
                ),
              ),
            );
          }
      ...
      if (_paymentMethod == PaymentMethod.momo ||
        ...
        try {
          ...
          await _showHandoffInterstitial(handoff);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Scan a table QR before placing an order.'),
        ),
      );
    }
  ...
  Future<void> _showHandoffInterstitial(PaymentHandoff handoff) async {
    final isMoMo = handoff.method == PaymentMethod.momo;
    await showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => AlertDialog(
        title: Text(isMoMo ? 'MoMo USSD handoff' : 'Revolut handoff'),
        content: Column(
          ...
          children: [
            ...
            const Text(
              'FANZONE does not confirm this payment automatically.',
        ...
        actions: [
          ElevatedButton(
            ...
            child: Text(isMoMo ? 'OPEN USSD' : 'OPEN REVOLUT'),
          ),
        ],
      ),
    );
    ...
    if (cart.isEmpty) {
      return Scaffold(
        ...
        body: StateView.empty(
          title: 'Cart is empty',
          subtitle: 'Add menu items from the Bar before placing an order.',
          icon: LucideIcons.shoppingCart,
          action: () => context.go('/bar'),
              ...
              child: Column(
                children: [
                  SwitchListTile.adaptive(
                    value: _useFetSpend,
                    onChanged: (value) => setState(() => _useFetSpend = value),
              ...
              children: [
                ...
                if (hasRevolutHandoff) ...[
                  ...
                  _PaymentMethodTile(
                    ...
                    onTap: () =>
                        setState(() => _paymentMethod = PaymentMethod.revolut),
                  ),
                ],
              ],
            ),
