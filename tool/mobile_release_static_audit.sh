...
require_contains "${android_manifest}" 'android:host="pools"' \
  "Android manifest must register fanzone://pools links generated by pool sharing."
...
require_contains "tool/android_deep_link_smoke.sh" 'https://fanzone\.guest\.ikanisa\.com/predict/\$\{POOL_SHARE_SLUG\}' \
  "Android deep-link smoke must cover legacy prediction links that normalize into pools."
require_contains "tool/android_deep_link_smoke.sh" 'FANZONE_DEEPLINK_SMOKE_RELEASE' \
